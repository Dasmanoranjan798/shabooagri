...
- **Inline Master-Data Creation (`CustomerFormModal`, `BookingFormModal`, `MachineFormModal`)**: Added `createVillage` and `createMachineType` methods to `api.ts` and built inline **"+ Quick Create"** toggles for Villages and Machine Types across Farmer, Booking, and Machine registration modals so users can create new master data on the fly.
- **Farmer Duplicate Validation & Save Error Fix (`CustomerFormModal.tsx` & `customer.service.ts`)**: Enforced server-side uniqueness checks on Farmer Mobile Phone number (`findByPhoneScoped`) and Farmer Name + Village combination (`findByNameAndVillageScoped`) per company. Solved post-save invite error catching so saved records are never misreported as failed.
- **Test Suite Multi-Tenant Data Isolation**: Refactored backend test suites (`test-payments.ts`, `test-pricing-methods.ts`) to operate inside dynamically created isolated test companies (`paytest-*`, `pricetest-*`). Test suite runs now zero out their own test companies on teardown without touching live company data or user-created farmer records.
