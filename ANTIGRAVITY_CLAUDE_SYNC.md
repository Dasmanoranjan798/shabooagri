...
- **Inline Master-Data Creation (`CustomerFormModal`, `BookingFormModal`, `MachineFormModal`)**: Added `createVillage` and `createMachineType` methods to `api.ts` and built inline **"+ Quick Create"** toggles for Villages, Farmers, and Machine Types across Farmer, Booking, and Machine registration modals so users can create new master data on the fly in a single submit click.
- **Complete Operational Database Purge & Fresh Reset**: Executed complete purge of all operational database records (Customers, Villages, Machines, MachineTypes, Drivers, Employees, Bookings, Jobs, Invoices, Payments, Expenses, Maintenance Records, Invites). The database is 100% clean and ready for fresh data creation.
