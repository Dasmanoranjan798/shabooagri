...
- **Farmer Creation & Village Auto-Link Fix (`CustomerFormModal.tsx`)**: Resolved `villageId` reset race condition and added automatic inline village creation on form submit. Creating a new farmer now automatically creates and links the village seamlessly if typed.
